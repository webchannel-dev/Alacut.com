...
if($memberID!="" && $memberID!=0){
	...
	if($_REQUEST['p']=='p')
	{
		...
		WHERE ".$alwmbr." u.status!= '2'  AND u.video_id != " . $iFrameVideoID." ORDER BY u.`visit` DESC,u.`like`,u.`uploaded_date` DESC LIMIT 0,4";
		$videoPostRes = mysql_query($videoPostQry);
	$videoPostNum = mysql_num_rows($videoPostRes);
	}
	else {
		$videoPostQry = "SELECT * FROM
				(	SELECT u.video_id,u.video_upload_by,u.video_object,u.video_name,u.video_type,u.visit,u.video_image,u.like,u.dislike,a.full_name 
						FROM `upload_video` u INNER JOIN `alacut_member` a ON a.member_id = u.video_upload_by
						INNER JOIN `friend` f1 ON f1.frnd_req_from_id =  u.video_upload_by		
						WHERE  u.status!= '2'  AND f1.status='C'  AND  f1.frnd_req_to_id=$memberID AND u.video_id != " . $iFrameVideoID."  
						UNION ALL
						SELECT u.video_id,u.video_upload_by,u.video_object,u.video_name,u.video_type,u.visit,u.video_image,u.like,u.dislike,a.full_name 
						FROM `upload_video` u INNER JOIN `alacut_member` a ON a.member_id = u.video_upload_by
						INNER JOIN `friend` f2 ON f2.frnd_req_to_id =  u.video_upload_by  
						WHERE  u.status!= '2'  AND  f2.status='C' AND  f2.frnd_req_from_id=$memberID AND u.video_id != " . $iFrameVideoID."
					) AS vw ORDER BY vw.visit DESC,vw.like DESC LIMIT 0,10";
		$videoPostRes = mysql_query($videoPostQry);
		$videoPostNum = mysql_num_rows($videoPostRes);
		if($videoPostNum == 0 )
		{
			$videoPostQry="SELECT u.video_id,u.video_upload_by,u.video_object,u.video_name,u.video_type,u.video_image,u.like,u.dislike,a.full_name 
			FROM `upload_video` u INNER JOIN `alacut_member` a ON a.member_id = u.video_upload_by 
			WHERE  u.status!= '2'  AND u.video_id != " . $iFrameVideoID." ORDER BY u.`visit` DESC,u.`like` DESC LIMIT 0,10";
			$videoPostRes = mysql_query($videoPostQry);
			$videoPostNum = mysql_num_rows($videoPostRes);
		}
		//echo $videoPostQry;
