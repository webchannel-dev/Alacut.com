<?php
	...
	{
		$to      = $email; //Send email to other than alacut user
		$subject = 'Friend Invitation for alacut social networking' ;
		$message = '

		Hi
		I’d like to add you on Alacut Social Networking.
		
		Click on the link to accept the friend request:
		http://project.bloochip.com/alacut-Live/script/home.php
	...
	//Vimeo video Description
	 ...
	 {	
        ...
        $result = curl_exec($ch);
        curl_close($ch);
	    $fp = fopen($_SERVER['DOCUMENT_ROOT'].'/test/upload/videoDetails/'.$output_filename, 'w'); 
        fwrite($fp, $result);
        fclose($fp);	
    ...
    {      
      ...
      echo $obj->description = $media->group->description;
	  exit();
