...
function unFriend(fromID,toID,src)
{
	$.ajax({
		...
		success: function(data) {
			...
			$('</div>').bind('click','addFriend('+fromID+','+toID+',this)').attr({"class":"button","href":"javascript:void(0);","style":"margin: 0;"}).text('Add as friend').appendTo($(src).parent());
			$(src).remove();
...
function addFriend(fromID,toID,src)
{
	$.ajax({
		...
		success: function(data) {
			$('</div>').bind('click','unFriend('+fromID+','+toID+',this)').attr({"class":"button","href":"javascript:void(0);","style":"margin: 0;"}).text('Unfriend').appendTo($(src).parent());
			$(src).remove();
		},
		error: function(e) {
			alert("There is somme error in the network. Please try later.");
...
<div class="inner_left">
  ...
  <div class="inner_right">
    ...
    <?php if($selectStatusInfo['status'] != 'C' || $selectStatusInfo['status'] != 'P') {?>
    <div class="button" onclick="addFriend('<?php echo $user_check;?>','<?php echo $_POST['id'];?>',this)"><a href="#" style="margin: 0;">Add as friend</a></div>
    <?php } else {?>
    <div class="button" onclick="unFriend('<?php echo $user_check;?>','<?php echo $_POST['id'];?>',this)"><a class="button" href="#" style="margin: 0;">Unfriend</a>
    <?php }?>                                            
    </div>
  </div>
